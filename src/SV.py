...
try:
    ...
    logging.getLogger('MEIsensor.dependencies').info('Dependency edlib is not installed - alignments are disabled.')
...
class SVGroup:
    ...
    def call(self, config, task) -> Optional[SVCall]:
        ...
        svcall_pos = int(utils.median(cand.pos for cand in self.candidates))
        svcall_svlen = int(utils.median(cand.svlen for cand in self.candidates))
        ...
        svcall = SVCall(contig=first_cand.contig,
                        ...
                        ref="N",
                        alt=svcall_alt,
                        qual=utils.mean_or_none_round(int(cand.qual) for cand in self.candidates if cand.qual is not None),
                        ...
                        support=round(utils.mean(cand.support for cand in self.candidates)),
                        rnames=rnames,
                        postprocess=None,
                        qc=True,
                        nm=-1,
                        ...
                        coverage_upstream=utils.mean_or_none_round(cand.coverage_upstream for cand in self.candidates if cand.coverage_upstream is not None),
                        coverage_start=utils.mean_or_none_round(cand.coverage_start for cand in self.candidates if cand.coverage_start is not None),
                        coverage_center=utils.mean_or_none_round(cand.coverage_center for cand in self.candidates if cand.coverage_center is not None),
                        coverage_end=utils.mean_or_none_round(cand.coverage_end for cand in self.candidates if cand.coverage_end is not None),
                        coverage_downstream=utils.mean_or_none_round(cand.coverage_downstream for cand in self.candidates if cand.coverage_downstream is not None))

        if n_samples != 1:
            svcall.set_info("STDEV_POS", utils.stdev(cand.pos for cand in self.candidates))
            svcall.set_info("STDEV_LEN", utils.stdev(cand.svlen for cand in self.candidates))
...
def call_from(cluster, config, keep_qc_fails, task):
    ...
    svlen = utils.center(v.svlen for v in leads)
    ...
    if svtype == "INS" and svlen >= config.long_ins_length:
        ...
        support = len(support_set)
        support_long = 0
    ref_start = utils.center(v.ref_start for v in leads)
    stdev_pos = utils.stdev(utils.trim((v.ref_start for v in leads)))
    precise = (stdev_pos < config.precise)
    svstart, svend = calculate_bounds(svtype, ref_start, svlen)
    qual = int(utils.mean(v.mapq for v in leads))
    ...
    if config.qc_nm_measure:
        nm_mean = utils.mean(v.nm for v in leads)
    else:
        nm_mean = -1
...
def merge_inner_bounds(leads, config):
    ...
    svlen = utils.center(v for k, v in read_svlengths.items())
    pos = utils.center(v for k, v in read_starts.items())
    ...
    return pos, svlen, utils.stdev(utils.trim((v for k, v in read_starts.items()))), utils.stdev(utils.trim((v for k, v in read_svlengths.items())))
